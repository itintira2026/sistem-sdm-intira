    ...
    <div class="py-12">
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                    ...
                    <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
                        <div class="p-4 rounded-lg bg-teal-50">
                            ...
                            <p class="mt-2 text-3xl font-bold text-teal-800">
                                {{ $stats['total_today'] }}<span class="text-lg text-teal-400">/{{ $stats['target'] }}</span>
                            </p>
                        ...
                        <div class="p-4 rounded-lg {{ $stats['report_12'] ? 'bg-green-50' : 'bg-orange-50' }}">
                            ...
                            <p class="mt-2 text-2xl font-bold {{ $stats['report_12'] ? 'text-green-800' : 'text-orange-800' }}">
                                @if ($stats['report_12'])
                                    ✅ Sudah
                                @else
                                    ⏳ Belum
                        ...
                        <div class="p-4 rounded-lg {{ $stats['report_16'] ? 'bg-green-50' : 'bg-orange-50' }}">
                            ...
                            <p class="mt-2 text-2xl font-bold {{ $stats['report_16'] ? 'text-green-800' : 'text-orange-800' }}">
                                @if ($stats['report_16'])
                                    ✅ Sudah
                                @else
                                    ⏳ Belum
                        ...
                        <div class="p-4 rounded-lg {{ $stats['report_20'] ? 'bg-green-50' : 'bg-orange-50' }}">
                            ...
                            <p class="mt-2 text-2xl font-bold {{ $stats['report_20'] ? 'text-green-800' : 'text-orange-800' }}">
                                @if ($stats['report_20'])
                                    ✅ Sudah
                                @else
                                    ⏳ Belum
            ...
            <form action="{{ route('daily-reports.3hour-manager.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-6 overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <h3 class="mb-4 text-lg font-semibold text-gray-800">Pilih Waktu <span class="text-red-500">*</span></h3>
                ...
                <div class="mb-6 overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        ...
                        <!-- Import Rahn (Disabled untuk sementara) -->
                        <div class="p-4 mb-4 transition rounded-lg opacity-50 bg-gray-50">
                            <div class="flex flex-col items-start gap-4 md:flex-row md:items-center">
                                <label class="text-sm font-medium text-gray-500 md:w-40">
                                    Import Rahn
                                </label>
                                <div class="flex-1 w-full">
                                    <div class="flex items-center justify-between w-full gap-3 p-3 bg-white border-2 border-gray-200 border-dashed rounded-lg cursor-not-allowed">
                                        <div class="flex items-center flex-1 gap-2">
                                            <span class="flex-shrink-0 w-2 h-2 bg-gray-400 rounded-full"></span>
                                            <span class="text-sm text-gray-400">Fitur akan segera tersedia</span>
                                        </div>
                                        <button type="button" disabled class="px-4 py-2 text-sm font-medium text-white bg-gray-400 rounded-lg cursor-not-allowed">
                                            Import File
                                        </button>
                                    </div>
                                </div>
                        ...
                        <!-- Import Nasabah (Disabled untuk sementara) -->
                        <div class="p-4 mb-4 transition rounded-lg opacity-50 bg-gray-50">
                            <div class="flex flex-col items-start gap-4 md:flex-row md:items-center">
                                <label class="text-sm font-medium text-gray-500 md:w-40">
                                    Import Nasabah
                        ...
                        <div class="p-4 transition rounded-lg bg-gray-50 hover:bg-gray-100">
                            <div class="flex flex-col items-start gap-4 md:flex-row md:items-center">
                                ...
                                <div class="flex-1 w-full">
                                    <div class="flex items-center justify-between w-full gap-3 p-3 bg-white border-2 border-gray-300 border-dashed rounded-lg cursor-pointer hover:border-teal-500 hover:bg-teal-50" onclick="document.getElementById('file-revenue').click()">
                                        <div class="flex items-center flex-1 gap-2">
                                            <span class="flex-shrink-0 w-2 h-2 bg-gray-400 rounded-full status-dot" id="dot-revenue"></span>
                                            <span class="text-sm text-gray-600 file-name" id="name-revenue">Belum ada file dipilih</span>
                                        </div>
                                        <button type="button" class="px-4 py-2 text-sm font-medium text-white transition bg-teal-600 rounded-lg hover:bg-teal-700">
                                            Import File
                                        </button>
                                    </div>
                                    <input type="file" name="file_revenue" id="file-revenue" class="hidden" onchange="handleFileSelect(event, 'revenue')" accept=".xlsx,.xls,.csv">
                                    <p class="mt-2 text-xs text-gray-500">Format: Excel (.xlsx, .xls) atau CSV. Max 2MB</p>
                                    <p class="mt-1 text-xs text-gray-500">Header: no_akad, jumlah_pembayaran, tanggal_transaksi, keterangan (optional)</p>
                                </div>
                            </div>
                            @error('file_revenue')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                ...
                <div class="mb-6 overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        ...
                        <textarea 
                            ...
                            rows="8"
                            placeholder="Masukkan keterangan di sini...">{{ old('keterangan') }}</textarea>
                        ...
                        <div class="flex justify-end gap-3 mt-4">
                            <a href="{{ route('daily-reports.3hour-manager.index') }}" class="px-6 py-2 text-sm font-medium text-gray-700 transition bg-white border border-gray-300 rounded-lg hover:bg-gray-50">
                                Batal
                            </a>
                            <button type="submit" class="px-6 py-2 text-sm font-medium text-white transition bg-teal-600 rounded-lg hover:bg-teal-700">
                                Simpan Laporan
                            </button>
                        </div>
                    </div>
