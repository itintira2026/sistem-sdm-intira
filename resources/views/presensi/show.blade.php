        ...
        <div class="max-w-5xl mx-auto space-y-6 sm:px-6 lg:px-8">
            ...
            <div class="p-5 bg-white rounded-lg shadow-sm">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div>
                        <p class="text-xs font-medium text-gray-500 uppercase">Karyawan</p>
                        <p class="mt-1 font-semibold text-gray-800">{{ $user->name }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-medium text-gray-500 uppercase">Cabang</p>
                        @if ($branch)
                            <p class="mt-1 font-semibold text-gray-800">{{ $branch->name }}</p>
                            <p class="text-xs text-gray-400">{{ $branch->address ?? '-' }} ·
                                {{ $branch->timezone ?? '-' }}</p>
                        @else
                    ...
                    <div>
                        <p class="mb-1 text-xs font-medium text-gray-500 uppercase">Filter Bulan</p>
            ...
            <div class="grid grid-cols-2 gap-4 md:grid-cols-4">
                <div class="p-4 bg-white border-l-4 border-green-500 rounded-lg shadow-sm">
                    ...
                    <p class="mt-1 text-2xl font-bold text-green-600">{{ $summary['total_hadir'] }}</p>
                ...
                <div class="p-4 bg-white border-l-4 border-yellow-500 rounded-lg shadow-sm">
                    ...
                    <p class="mt-1 text-2xl font-bold text-yellow-600">{{ $summary['total_terlambat'] }}</p>
                    <p class="mt-1 text-xs text-gray-400">hari</p>
                </div>
                <div class="p-4 bg-white border-l-4 border-blue-500 rounded-lg shadow-sm">
                    ...
                    <p class="mt-1 text-2xl font-bold text-blue-600">{{ $summary['total_izin'] }}</p>
                    <p class="mt-1 text-xs text-gray-400">hari</p>
                </div>
                <div class="p-4 bg-white border-l-4 border-red-500 rounded-lg shadow-sm">
                    ...
                    <p class="mt-1 text-2xl font-bold text-red-600">{{ $summary['total_alpha'] }}</p>
                    <p class="mt-1 text-xs text-gray-400">hari</p>
                </div>
            </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border-l-4 {{ $borderColor }} overflow-hidden">
                    ...
                    <button type="button" onclick="toggleAccordion('acc-{{ $loop->index }}')"
                        ...
                        <div class="flex items-center flex-shrink-0 gap-2 ml-2">
                            ...
                            @endif
                            <svg class="w-5 h-5 text-gray-400 transition-transform duration-200 transform"
                    ...
                    <div id="acc-{{ $loop->index }}"
                        ...
                        <div class="p-5 space-y-5">
                            ...
                            <div class="flex flex-col gap-5">
                                    ...
                                    <div class="space-y-4">
                                        ...
                                        @foreach ($statusList as $s)
                                            <div class="relative flex items-start gap-4">
                                                <div
                                                    class="w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 z-10 {{ $s['item'] ? $s['dot'] : 'bg-gray-200' }}">
                                                    @if ($s['item'])
                                                ...
                                                <div class="flex-1 pb-1">
                                                    <div class="flex items-center justify-between">
                                                            ...
                                                            <div class="flex items-center gap-2">
                                                                ...
                                                                <div class="relative inline-block">
                                                                    ...
                                                                    <div id="dropdown-{{ $s['item']->id }}"
                                                                        class="fixed z-50 hidden bg-white rounded-lg shadow-lg w-44 ring-1 ring-black ring-opacity-5">
                                                                        <div class="py-1">
                                                                            @if ($s['item']->photo)
                                                                                <button
                                                                                    onclick="openPhoto('{{ Storage::url($s['item']->photo) }}', '{{ $s['label'] }}', '{{ \Carbon\Carbon::parse($s['item']->jam)->format('H:i') }}')"
                                                                            ...
                                                                            @endif
                                                                            <button
                                                                                onclick="openEditModal('{{ $s['item']->status }}', '{{ \Carbon\Carbon::parse($s['item']->jam)->format('H:i:s') }}', '{{ $s['item']->wilayah }}', '{{ $s['item']->keterangan }}', '{{ $tanggal }}')"
                                                                                class="flex items-center w-full gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                                                            ...
                                                                            <form
                                                                                action="{{ route('presensi.destroy', $s['item']->id) }}"
                                                                                method="POST">
                                                                                @csrf
                                                                                @method('DELETE')
                                                                                <button type="submit"
                                                        ...
                                                        @else
                                                            <button
                                                                onclick="openEditModal('{{ $s['label'] === 'Check In' ? 'CHECK_IN' : ($s['label'] === 'Check Out' ? 'CHECK_OUT' : ($s['label'] === 'Istirahat In' ? 'ISTIRAHAT_IN' : 'ISTIRAHAT_OUT')) }}', '', '', '', '{{ $tanggal }}')"
                                                    ...
                                                    @if ($s['item'])
                                                        <div
                                                            ...
                                                            @if ($s['item']->keterangan)
                                                                <span
                                                                    class="italic">{{ $s['item']->keterangan }}</span>
                                                            @endif
                                                        </div>
                                                    @endif
                                                </div>
                                ...
                                @php
                                    $photosToShow = $entries->filter(fn($e) => !empty($e->photo))->values();
                                    $outfitPhoto = $entries->where('status', 'CHECK_IN')->first()?->photo_outfit; // tambah ini
                                    $closingFotos = $entries // tambah ini
                                        ...
                                        <div class="flex flex-row flex-wrap gap-3">
                                            @if ($outfitPhoto)
                                                <div class="relative cursor-pointer group"
                                                    ...
                                                    <p class="mt-1 text-xs text-center text-purple-400">Outfit</p>
                                                </div>
                                            @endif
                                        ...
                                        <div class="flex flex-row flex-wrap gap-3">
                                            @foreach ($closingFotos as $closing)
                                                <div class="relative cursor-pointer group"
                                                    ...
                                                    <p class="mt-1 text-xs text-center text-orange-400">
                                                        {{ $closing->kategori ?? '-' }}
                                                    </p>
                                                </div>
                                            @endforeach
            ...
            @empty
                <div class="p-12 text-center bg-white rounded-lg shadow-sm">
                    ...
                    <p class="font-medium text-gray-400">Tidak ada data presensi</p>
                    <p class="mt-1 text-sm text-gray-300">
                        Belum ada presensi di bulan
            ...
            <form method="POST" action="{{ route('presensi.export.user', $user->id) }}">
                ...
                <div class="grid grid-cols-3 gap-2 mb-4">
                    <button type="button" onclick="setExportRange('week')"
                        class="text-xs py-1.5 px-2 border border-gray-300 rounded-lg hover:bg-gray-50 text-gray-600">Minggu
                        Ini</button>
                    <button type="button" onclick="setExportRange('month')"
                        class="text-xs py-1.5 px-2 border border-gray-300 rounded-lg hover:bg-gray-50 text-gray-600">Bulan
                        Ini</button>
        ...
        <div class="w-full max-w-md bg-white rounded-lg shadow-xl" onclick="event.stopPropagation()">
            ...
            <form method="POST" action="{{ route('presensi.update', $user->id) }}">
                ...
                <div class="p-6 space-y-4">
                    ...
                    <div>
                        <label class="block mb-2 text-sm font-medium text-gray-700">Jam <span
